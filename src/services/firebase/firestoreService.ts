...
class FirestoreService {
  /**
   * Eliminar campos undefined de un objeto (Firestore no los acepta)
   */
  private limpiarDatos<T>(datos: T): any {
    const datosFiltrados: any = {};
    
    Object.keys(datos as any).forEach((key) => {
      const valor = (datos as any)[key];
      if (valor !== undefined) {
